...
function buildCommandList() {
    return [
        ...
        new SlashCommandBuilder()
            .setName('ticket')
            .setDescription('Système de tickets support')
            .addSubcommand(s => s.setName('panel').setDescription('Envoie le panel de création de tickets (admin)'))
            .addSubcommand(s => s.setName('close').setDescription('Ferme le ticket actuel'))
            .addSubcommand(s => s.setName('claim').setDescription('Prend en charge le ticket actuel (staff)'))
            .addSubcommand(s => s.setName('add').setDescription('Ajoute un membre au ticket (staff)')
                .addUserOption(o => o.setName('membre').setDescription('Membre à ajouter').setRequired(true)))
            .addSubcommand(s => s.setName('remove').setDescription('Retire un membre du ticket (staff)')
                .addUserOption(o => o.setName('membre').setDescription('Membre à retirer').setRequired(true)))
            .addSubcommand(s => s.setName('rename').setDescription('Renomme le ticket (staff)')
                .addStringOption(o => o.setName('nom').setDescription('Nouveau nom').setRequired(true)))
            .addSubcommand(s => s.setName('info').setDescription('Affiche les infos du ticket actuel'))
            .addSubcommand(s => s.setName('config').setDescription('Lien vers le dashboard de configuration (admin)'))
            .toJSON(),
    ];
}
