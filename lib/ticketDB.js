...
async function init(poolInstance) {
    ...
    await pool.execute(`
        CREATE TABLE IF NOT EXISTS support_staff_stats (
            id              BIGINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
            ticket_id       VARCHAR(8)   NOT NULL,
            guild_id        VARCHAR(20)  NOT NULL,
            staff_id        VARCHAR(20)  NOT NULL,
            staff_pseudo    VARCHAR(100) DEFAULT NULL,
            message_count   INT UNSIGNED NOT NULL DEFAULT 0,
            first_message_at TIMESTAMP   NULL DEFAULT NULL,
            ticket_created_at TIMESTAMP  NULL DEFAULT NULL,
...
async function deleteTicket(id) {
    ...
    try {
        ...
        await conn.execute('DELETE FROM support_stats       WHERE ticket_id = ?', [id]);
        await conn.execute('DELETE FROM support_staff_stats WHERE ticket_id = ?', [id]);
        await conn.execute('DELETE FROM support_tickets      WHERE id = ?',       [id]);
        await conn.commit();
    } catch (err) {
        await conn.rollback();
        throw err;
...
function assertPool() {
    if (!pool) throw new Error('ticketDB pool non initialisé — DB pas encore connectée');
}
...
async function getStats(guildId, days = 30) {
    const [[opened], [closed], [byCategory], [totals]] = await Promise.all([
        pool.execute(
...
async function recordStaffMessage(ticketId, guildId, staffId, staffPseudo, ticketCreatedAt) {
    // UPSERT: first message sets first_message_at, subsequent ones only increment count.
    await pool.execute(`
        INSERT INTO support_staff_stats
            (ticket_id, guild_id, staff_id, staff_pseudo, message_count, first_message_at, ticket_created_at)
        VALUES (?, ?, ?, ?, 1, NOW(), ?)
        ON DUPLICATE KEY UPDATE
            message_count    = message_count + 1,
            staff_pseudo     = VALUES(staff_pseudo)
    `, [ticketId, guildId, staffId, staffPseudo || null, ticketCreatedAt || null]);
}

async function getStaffStats(guildId) {
    // Increase GROUP_CONCAT limit for this session to avoid silent truncation
    // at the default 1024-byte cap when a staff member handles many tickets.
    await pool.execute('SET SESSION group_concat_max_len = 65536');
    const [rows] = await pool.execute(`
        SELECT
            staff_id,
            MAX(staff_pseudo)                                                   AS staff_pseudo,
            COUNT(DISTINCT ticket_id)                                           AS ticket_count,
            SUM(message_count)                                                  AS total_messages,
            GROUP_CONCAT(DISTINCT ticket_id ORDER BY ticket_id SEPARATOR ',')  AS ticket_ids,
            AVG(TIMESTAMPDIFF(SECOND, ticket_created_at, first_message_at))    AS avg_response_seconds
        FROM support_staff_stats
        WHERE guild_id = ?
        GROUP BY staff_id
    ...
    return rows.map(r => ({
        staff_id:            r.staff_id,
        staff_pseudo:        r.staff_pseudo || r.staff_id,
        ticket_count:        Number(r.ticket_count),
        total_messages:      Number(r.total_messages),
        ticket_ids:          r.ticket_ids ? r.ticket_ids.split(',') : [],
        avg_response_seconds: r.avg_response_seconds != null ? Math.round(Number(r.avg_response_seconds)) : null,
    }));
}
