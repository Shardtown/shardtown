...
// MySQL rejects ISO-8601 strings (e.g. "2026-06-09T19:39:36.000Z").
// Convert any date/datetime/timestamp value to "YYYY-MM-DD HH:MM:SS".
function normalizeValue(val, colType) {
    if (val === null || val === undefined || val === '') return val;
    const t = (colType || '').toLowerCase();
    if (t === 'date') {
        // Accept ISO or any parseable date → YYYY-MM-DD
        try {
    ...
    if (t === 'datetime' || t === 'timestamp') {
        // Accept ISO-8601 or MySQL format → "YYYY-MM-DD HH:MM:SS"
        try {
            const d = new Date(val);
            if (!isNaN(d.getTime())) {
...
// Apply normalizeValue to every column in a data object.
function normalizeRow(obj, schema) {
    const result = { ...obj };
    for (const col of schema) {
        if (Object.prototype.hasOwnProperty.call(result, col.name)) {
            result[col.name] = normalizeValue(result[col.name], col.type);
        }
    }
    return result;
}
...
module.exports = function mountAdminDB(app, getDb, checkAdmin, verifyCsrf, logAdminAction) {
    ...
    app.post('/api/admin/db/:table/row', checkAdmin, verifyCsrf, mutateLimiter, async (req, res) => {
        ...
        try {
            ...
            const data   = normalizeRow(raw, schema);
    ...
    app.put('/api/admin/db/:table/row', checkAdmin, verifyCsrf, mutateLimiter, async (req, res) => {
        ...
        try {
            ...
            const schema = await getSchema(db, name);
            const { pk, set: rawSet } = req.body || {};
            if (!pk || !rawSet) return res.status(400).json({ error: 'Champs pk et set requis' });

            const set     = normalizeRow(rawSet, schema);
