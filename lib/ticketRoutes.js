...
router.get('/guild/:guildId', requireAuth, async (req, res) => {
    ...
    if (req.session?.isAdmin) {
        // On n'a pas les infos du serveur sans appel API, on les récupère
        try {
            const r = await axios.get(`https://discord.com/api/v10/guilds/${guildId}`, {
                headers: { Authorization: `Bot ${process.env.SHARD_TOKEN}` },
                timeout: 5000,
                validateStatus: s => s === 200 || s === 404,
            });
            if (r.status === 404) return res.status(403).json({ error: 'Serveur introuvable' });
            return res.json({ id: r.data.id, name: r.data.name, icon: r.data.icon || null });
    ...
    // 3. Fallback live : guilds_json vide ou guild absente du cache
    try {
        const [guildRes, memberRes] = await Promise.all([
            ...
            axios.get(`https://discord.com/api/v10/guilds/${guildId}/members/${user.id}`, {
                headers: { Authorization: `Bot ${process.env.SHARD_TOKEN}` },
                timeout: 5000,
                validateStatus: s => s === 200 || s === 404,
            }),
        ...
        // Vérification permissions live (MANAGE_GUILD ou ADMINISTRATOR)
        const roleMap = new Map(guildRes.data.roles.map(r => [r.id, BigInt(r.permissions || 0)]));
        let perms = roleMap.get(guildId) || 0n; // rôle @everyone = même ID que la guild
        ...
        console.error('[ticketRoutes] guild access check:', err.message);
