...
function requireAuth(req, res, next) {
    if (!getAuthUser(req)) {
        // Requête navigateur (pas AJAX) → redirection login avec returnTo
        const isAjax = req.xhr || (req.headers['accept'] || '').includes('application/json');
        if (!isAjax) {
            const base     = process.env.MAIN_SITE_URL || process.env.BASE_URL || 'https://shardtwn.fr';
            const returnTo = encodeURIComponent(`https://support.shardtwn.fr${req.originalUrl}`);
            return res.redirect(`${base}/shard/login?returnTo=${returnTo}`);
        }
