...
function attach({ client, db, captchaStore, tempChannels, helpers }) {
    ...
    if (interaction.isChatInputCommand()) {
        ...
        // ----- /promo (owner only) ---------------------------------------
        if (cmd === 'promo') {
            // Restreint au(x) bot-owner(s). SHARDTOWN_OWNER_ID peut être une
            // liste séparée par des virgules. Si pas défini, on refuse tout
            // (sécurité par défaut).
            ...
            try {
                ...
                if (sub === 'list') {
                    ...
                    // Coupe à 25 codes pour rester sous la limite 2000 chars Discord.
