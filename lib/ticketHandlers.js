...
function isTicketInteraction(interaction) {
    ...
    if (interaction.isStringSelectMenu())  return interaction.customId === 'ticket:pick_cat';
...
async function handle(interaction, client) {
    try {
        ...
        if (interaction.isStringSelectMenu() && interaction.customId === 'ticket:pick_cat') {
...
function buildTicketModal(category) {
    const title = `${category.emoji ? category.emoji + ' ' : ''}${category.label}`.slice(0, 45);
    const modal  = new ModalBuilder()
        .setCustomId(`ticket:modal:${category.id}`)
        .setTitle(title);

    const fields = category.modal_fields;
    if (fields && fields.length > 0) {
        for (const f of fields.slice(0, 5)) {
            ...
            if (f.min_length > 0) input.setMinLength(f.min_length);
            if (f.max_length > 0) input.setMaxLength(Math.min(f.max_length, 4000));
...
async function handleModal(interaction, client) {
    ...
    const fields = category?.modal_fields;
    if (fields && fields.length > 0) {
        description = fields.slice(0, 5).map(f => {
...
// Delegate staff message tracking to manager (called from SIH messageCreate)
function handleMessage(message) {
    return manager.handleStaffMessage(message);
}

module.exports = { isTicketInteraction, handle, attach, handleMessage };
