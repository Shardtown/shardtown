...
async function createTicketChannel(guild, ticket, category, config) {
    ...
    const channelOptions = {
        name: `ticket-${ticket.id.toLowerCase()}`,
...
async function buildPanel(config) {
    ...
    for (let i = 0; i < categories.length; i += chunkSize) {
        ...
        for (const cat of chunk) {
            ...
            if (cat.emoji && !cat.emoji.includes(':')) btn.setEmoji(cat.emoji);
...
async function openTicket(interaction, categoryId, description) {
    ...
    let channel;
    try {
        channel = await createTicketChannel(guild, { id: ticketId, authorId: interaction.user.id }, category, config);
    } catch (err) {
