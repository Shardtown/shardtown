...
async function createTicketChannel(guild, ticket, category, config) {
    ...
    const discordCategoryId = category.discord_category_id || category.categoryId || null;
...
async function getGuildConfig(guildId) {
    ...
    return {
        ...
        afk_timeout_minutes:   raw.afk_timeout_minutes != null ? Number(raw.afk_timeout_minutes) : 60,
        ...
        welcome_title:         raw.welcome_title         || 'Ticket #{id}',
        welcome_color:         raw.welcome_color         || '',
        welcome_footer:        raw.welcome_footer        || 'ID: {id}',
        claim_enabled:         raw.claim_enabled         !== undefined ? Boolean(raw.claim_enabled) : true,
        channel_name_format:   raw.channel_name_format   === 'id' ? 'id' : 'pseudo',
    };
}
...
async function buildPanel(config) {
    ...
    for (const cat of categories) {
        ...
        if (cat.emoji) {
            const customEmoji = cat.emoji.match(/^<(a?):([^:]+):(\d+)>$/);
            if (customEmoji) {
                opt.setEmoji({ name: customEmoji[2], id: customEmoji[3], animated: !!customEmoji[1] || undefined });
            } else {
...
async function openTicket(interaction, categoryId, description) {
    ...
    // Channel name : pseudo (sanitized) ou ID du ticket selon channel_name_format
    const channelName = config.channel_name_format === 'id'
    ...
    const welcomeColorHex = (config.welcome_color || config.panel_color || category.color || '#7c3aed').replace('#', '');
    const welcomeColor = parseInt(welcomeColorHex, 16) || 0x7c3aed;
    const welcomeTitle  = (config.welcome_title  || 'Ticket #{id}').replace('{id}', ticketId);
    const welcomeFooter = (config.welcome_footer || 'ID: {id}').replace('{id}', ticketId);
    ...
    const welcomeEmbed = new EmbedBuilder()
        ...
        .addFields(
            ...
            { name: 'Catégorie',   value: `${category.emoji || ''} ${category.label}`.trim(), inline: true },
    ...
    return all.map(msg => ({
        ...
        mentions: {
            users: [...msg.mentions.users.values()].map(u => ({
                id: u.id,
                name: u.username,
                displayName: msg.guild?.members.cache.get(u.id)?.displayName || u.username,
            })),
...
// Called on every messageCreate in a ticket channel.
// Silently ignored if the sender is not staff or the channel is not a ticket.
...
async function handleStaffMessage(message) {
    try {
        ...
        console.error('[ticket] handleStaffMessage:', err.message);
    }
}
