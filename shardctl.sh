...
update_menu() {
    while true; do
        clear_screen
        header
        echo -e "  ${BOLD}${WHITE}MISE À JOUR${NC}"
        echo -e "  ${DIM}──────────────────────────────────────────────${NC}"
        echo ""
        echo -e "  ${YELLOW}[1]${NC} Pull depuis GitHub + redémarrer tout"
        ...
        case $choice in
            1)
                ...
                cd $DIR && git pull
                echo -e "  ${CYAN}🔄 Redémarrage de tous les services...${NC}"
                pm2 restart all
                pm2 save
